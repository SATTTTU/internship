...
const Services = () => {
  ...
  const getTextColor = (bgColor) => {
    ...
    if (bgColor === "bg-[#B9FF66]" || bgColor === "bg-gray-50") {
      return "text-black"; // Light background, use black text
    } else {
      return "text-white"; // Dark background, use white text
    }
  };
          ...
          {cards.map((item, index) => (
            ...
            >
              <div className="p-6 flex-1 flex flex-col justify-between">
                ...
                <button className="flex items-center mt-4 space-x-2 text-black-500 hover:underline  transition duration-200 lg:absolute lg:bottom-6 lg:left-6">
                  <Btn />
                  <span className={`hidden lg:block ${getTextColor(item.bgColor)}`}>
                    Learn More
                  </span>
                </button>
              </div>
